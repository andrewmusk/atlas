...
CMD ["sh", "-c", "if [ -n \"$DATABASE_URL\" ]; then npx prisma db push --accept-data-loss && npx prisma db seed; else echo 'WARNING: DATABASE_URL not set, skipping schema sync'; fi && node server/index.js"]
